...
namespace TinyGraphics {
  ...
  struct Cone::Impl {
    ...
    real intersect_obj(const Ray& ray, Vector3D& normal, int& plane_idx) const {
      ...
      real h2 = h*h, r2 = r*r, h_y0 = h-y0;
      real r2_h2 = r2/h2;
      real a = x1*x1 + z1*z1 - r2_h2 * y1*y1;
      real b = 2 * (x0*x1 + z0*z1 + r2_h2 * h_y0 * y1);
      real c = x0*x0 + z0*z0 - r2_h2 * h_y0*h_y0;
      ...
      if (is_hit) {
        ...
        normal.assign(hit_12.x, r2_h2 * (h - hit_12.y), hit_12.z);
        normal.normalize();
      }
    ...
    Point2D get_plane_coord_obj(int plane_idx, const Point3D& point) const {
      if (plane_idx == 1) {
        real v = point.y/h;
        real r1 = (1 - v) * r;
        if (abs(r1) > TG_EPSILON) {
          real cos_theta = -point.z / r1;
          real theta;
          if (abs(cos_theta) <= 1) {
        ...
        } else {                    // the top of cone
          return Point2D(1, 1);
        }
      } else
      if (plane_idx == 2) {
        return Point2D(point.x/(2*r) + 0.5, point.z/(2*r) + 0.5);
      }
      return Point2D(-1, -1);
    }
    ...
    bool contain_obj_coord(const Point3D& point) {
      if (0 <= point.y && point.y <= h) {
        real r1 = r * (1 - point.y/h);
        if (point.x*point.x + point.z*point.z <= r1*r1) return true;
      }
      return false;
    }
    ...
    void update_triangle_set() {
      ...
      int vertex_size = 2*lx + 2;
      int triangle_size = 2*lx;
      ...
      for (i = 0; i < lx; ++i) {
        ...
        real x = r * cos(theta);
        real z = -r * sin(theta);
        Point3D vertex = Point3D(x, 0, z);
        canonical_triangle_set.vertices.push_back(vertex);
        canonical_triangle_set.vertices.push_back(vertex);
        Vector3D normal = Vector3D(x, ny, z);
        normal.normalize();
      ...
      for (i = 0; i < triangle_size; i += 2) {
        int b = i+1, c = (i == triangle_size - 2) ? 1 : b+2;
        canonical_triangle_set.triangles.push_back(Triangle(0, b, c));
        canonical_triangle_set.triangles.push_back(Triangle(btm_idx, c+1, b+1));
      }
  ...
  Cone::Cone(real r, real h)
    : pimpl(new Impl(r, h)) {}

  Cone::Cone(real r, real h, const Position& position)
  ...
  const TriangleSet& Cone::to_triangle_set() const {
    return pimpl->triangle_set;
  }

  real Cone::intersect(const Ray& ray, Vector3D& normal, int& plane_idx) const {
  ...
  Point2D Cone::get_plane_coord(int plane_idx, const Point3D& point) const {
    return pimpl->get_plane_coord_obj(plane_idx, pimpl->to_object_coord(point));
  }

  bool Cone::contain(const Point3D& point) const {
    Point3D obj_point = pimpl->to_object_coord(point);
    return pimpl->contain_obj_coord(obj_point);
  }

  const Material& Cone::get_material() const {
    return pimpl->material;
  }

  void Cone::set_material(const Material& material) {
    pimpl->material = material;
  }

  const Position& Cone::get_position() const {
    return pimpl->position;
  }

  void Cone::set_position(const Position& position) {
    pimpl->position = position;
    pimpl->update_position();
  }

  void Cone::get_radius_height(real& radius, real& height) const {
    radius = pimpl->r;  height = pimpl->h;
  }

  void Cone::set_radius_height(real radius, real height) {
    pimpl->r = radius;
    pimpl->h = height;
    pimpl->update_triangle_set();
  }

  int Cone::get_resolution() const {
    return pimpl->lx - MINIMAL_LX;
  }

  void Cone::set_resolution(int lx) {
